...
Page({
  ...
  onLoad: function (option) {
    ...
    var myac = app.globalData.activities;
    var my = [];
    for (var i=0;i<myac.length;i++) {
      if (myac[i].number == 1){
        my.push(myac[i])
      }
    }
    ...
    while (i < agd.length) {
        let c = agd[i].course;
        let l = agd[i].location;
        let j = i + 1;
        let end = parseInt(agd[i].time) + 1;
        while (j < agd.length) {
            if (agd[j].course === c && agd[j].location === l) {
              delete agd[j];
                end += 1;
            } else {
                break;
  ...
  navigate:function(e){
    app.globalData.selectedPlace = e.currentTarget.dataset.location;
